...
- [SEO] Quick wins del Pilar 1 implementados: `lib/site.ts` (constantes), `app/robots.ts`, `app/sitemap.ts`, `app/manifest.ts`, `app/opengraph-image.tsx`, `app/icon.svg`, `components/seo/JsonLd.tsx` (Organization + SoftwareApplication + WebSite), `next.config.ts` (headers/compression). Dominio de referencia: `https://www.awki-ai.cl` (confirmar en producción). Informe completo en `SEO_Audit.md`.
